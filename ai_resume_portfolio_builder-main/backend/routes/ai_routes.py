...
def generate_resume_endpoint():
    ...
    try:
        ...
        # Extract format and template from request body
        export_format = data.get('format', 'txt').lower()
        template_name = data.get('templateName', 'professional').lower()  # Fixed: was 'template'
        ...
        # Get profile data from request body (sent by React frontend)
        profile_data = data.get('profileData', session.get('profile', {}))
